...
def create_sector_filter(filter_id, column, dataset_id):
    ...
    # Configuration pour permettre la sélection multiple de secteurs
    # EQUALS : opérateur de correspondance
    ...
    sector_filter.add_custom_filter_list_configuration(
        ...
        category_values=[],  # Vide par défaut, sera rempli par le contrôle de paramètre
...
def create_year_timerange_filter(filter_id, column, dataset_id):
    ...
    # Définir la granularité temporelle à l'année
...
def create_filter_group(group_id, filters, sheet_id):
    ...
    # Configurer la portée : appliquer à tous les visuels de la feuille spécifiée
