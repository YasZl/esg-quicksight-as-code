...
def make_emissions_by_sector_bar(visual_id, dataset_id, mappings):
    ...
    bar.set_scroll_bar_visibility("VISIBLE")
    # X axis = sector
...
def make_emissions_over_time_line(visual_id, dataset_id, mappings):
    ...
    line.set_scroll_bar_visibility("VISIBLE")
    # X axis = date/year
...
def make_emissions_table(visual_id, dataset_id, mappings):
    ...
    # Default sort: highest emitters first
    table.add_field_sort(mappings["emissions_total"], "DESC")
...
# Helper â€“ build_basic_esg_visuals
